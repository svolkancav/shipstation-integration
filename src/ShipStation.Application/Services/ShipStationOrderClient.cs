...
internal sealed class ShipStationOrderClient : IShipStationOrderClient
{
    ...
    public async IAsyncEnumerable<OrderModel> EnumerateOrdersAsync(
        OrderQuery query,
    ...
    public async Task<OrderModel> CreateOrUpdateOrderAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var order = await ReadAsync<OrderModel>(response, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<bool> DeleteOrderAsync(long orderId, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogDebug("OrderModel {OrderId} was already absent from ShipStation", orderId);
            return false;
        }
