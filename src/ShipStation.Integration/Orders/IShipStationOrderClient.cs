...
public interface IShipStationOrderClient
{
    ...
    /// </summary>
    IAsyncEnumerable<Order> EnumerateOrdersAsync(OrderQuery query, CancellationToken cancellationToken = default);

    /// <summary>
    /// Creates or updates an order. ShipStation matches on
    /// <see cref="CreateOrderRequest.OrderKey"/> when one is supplied.
    /// </summary>
    Task<Order> CreateOrUpdateOrderAsync(CreateOrderRequest request, CancellationToken cancellationToken = default);
